...
class ViewIterationTestCase(testutil.TempDatabaseMixin, unittest.TestCase):
    ...
    def docfromnum(self, num):
        return {'_id': str(num), 'num': int(num / 2)}
    ...
    def test_batchsizes(self):
        ...
        self.assertRaises(ValueError, self.db.iterview('test/nums', 0).__next__)
        self.assertRaises(ValueError, self.db.iterview('test/nums', -1).__next__)
    ...
    def test_limit(self):
        ...
        self.assertRaises(ValueError, self.db.iterview('test/nums', 10, limit=0).__next__)
