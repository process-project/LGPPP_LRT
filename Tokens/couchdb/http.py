...
if sys.version < '2.7':
    ...
    from http.client import CannotSendHeader, _CS_REQ_STARTED, _CS_REQ_SENT
...
class Session(object):
    ...
    def request(self, method, url, body=None, headers=None, credentials=None,
        ...
        if (body is not None and not isinstance(body, str) and
...
class Cache(object):
    ...
    def _clean(self):
        ls = sorted(iter(self.by_url.items()), key=cache_sort)
